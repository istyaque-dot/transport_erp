...
# ==========================================
# 🚀 SUPABASE DATABASE CONNECTION (V2)
# ==========================================
...
# ==========================================
# 📤 FILE UPLOAD (Google Drive - Unchanged)
# ==========================================
...
def prepare_pod_file(uploaded_files):
    ...
    for file in uploaded_files:
        if file.name.lower().endswith((".jpg", ".jpeg", ".png")):
            ...
            try: img.thumbnail((A4_WIDTH, A4_HEIGHT), Image.Resampling.LANCZOS)
            except AttributeError: img.thumbnail((A4_WIDTH, A4_HEIGHT), Image.LANCZOS)
    ...
    if a4_images:
        ...
        if len(a4_images) == 1: a4_images[0].save(pdf_bytes, format="PDF", resolution=300)
        else: a4_images[0].save(pdf_bytes, format="PDF", resolution=300, save_all=True, append_images=a4_images[1:])
...
def save_gr_link_to_db(trip_id, gr_url):
    try:
        supabase.table("bookings").update({"gr_link": gr_url}).eq("trip_id", trip_id).execute()
        return True
    except: return False

def save_booking_to_db(data_dict):
    try:
        supabase.table("bookings").insert(data_dict).execute()
        return True
    except: return False

@st.cache_data(ttl=15) # Cache time reduced because Supabase is super fast
def get_all_trips():
    try:
        res = supabase.table("bookings").select("*").execute()
        if res.data:
            df = pd.DataFrame(res.data)
            # Ensuring old UI layout doesn't break
            cols = ['date_val', 'from_loc', 'company', 'owner_rate', 'comp_rate', 'weight',
                    'truck_no', 'to_loc', 'gr_no', 'uni_amt', 'comments', 'comp_freight',
                    'owner_freight', 'final_uni_amt', 'trip_id', 'ish_amt', 'gr_link']
            for c in cols:
...
def update_booking_in_db(trip_id, data_dict):
    try:
        supabase.table("bookings").update(data_dict).eq("trip_id", trip_id).execute()
        return True
    except: return False
...
def save_to_ledgers(date_val, trip_id, gr_no, truck_no, dest, comp_amt, owner_amt, uni_amt, ish_amt):
    try:
        ...
        supabase.table("company_ledger").insert({"date_val": str(date_val), "trip_id": str(trip_id), "gr_no": gr, "truck_no": str(truck_no), "description": str(dest), "amount": int(comp_amt)}).execute()
        supabase.table("owner_ledger").insert({"date_val": str(date_val), "trip_id": str(trip_id), "gr_no": gr, "truck_no": str(truck_no), "description": str(dest), "amount": int(owner_amt)}).execute()
        if int(uni_amt) > 0:
            supabase.table("universal_ledger").insert({"date_val": str(date_val), "trip_id": str(trip_id), "gr_no": gr, "truck_no": "N/A", "description": f"Freight: {truck_no}", "amount": -int(uni_amt)}).execute()
        if int(ish_amt) > 0:
            supabase.table("ishtyaque_ledger").insert({"date_val": str(date_val), "trip_id": str(trip_id), "gr_no": gr, "truck_no": "N/A", "description": f"Profit: {truck_no}", "amount": -int(ish_amt)}).execute()
        return True
    except: return False
...
def update_ledgers(date_val, trip_id, gr_no, truck_no, dest, comp_amt, owner_amt, uni_amt, ish_amt):
    try:
        gr = str(gr_no).strip() if str(gr_no).strip() else "N/A"
        for table, amt, desc in [
            ("company_ledger", int(comp_amt), str(dest)),
            ("owner_ledger", int(owner_amt), str(dest)),
            ("universal_ledger", -int(uni_amt) if int(uni_amt) > 0 else 0, f"Freight: {truck_no}"),
            ("ishtyaque_ledger", -int(ish_amt) if int(ish_amt) > 0 else 0, f"Profit: {truck_no}")
        ]:
            if amt == 0 and table in ["universal_ledger", "ishtyaque_ledger"]: continue
            
            res = supabase.table(table).select("id").eq("trip_id", trip_id).order("id").limit(1).execute()
            if res.data:
                row_id = res.data[0]["id"]
                supabase.table(table).update({"date_val": str(date_val), "gr_no": gr, "truck_no": str(truck_no) if table in ["company_ledger", "owner_ledger"] else "N/A", "description": desc, "amount": amt}).eq("id", row_id).execute()
            else:
                supabase.table(table).insert({"date_val": str(date_val), "trip_id": str(trip_id), "gr_no": gr, "truck_no": str(truck_no) if table in ["company_ledger", "owner_ledger"] else "N/A", "description": desc, "amount": amt}).execute()
        return True
    except: return False

# ==========================================
# 🎨 CSS (Micro-adjusted for Mac)
# ==========================================
BOOKING_CSS = """
<style>
...
def show_booking_page():
    ...
    st.header("🚛 बुकिंग मैनेजमेंट (V2 Superfast)")
    ...
    with tab1:
        if not st.session_state.show_confirm:
            with st.form(key=f"booking_form_{c}"):
                ...
                with r2c2: weight     = st.number_input("⚖️ माल का वज़न (क्विंटल)", min_value=0.0, step=1.0)
                with r2c3: comp_rate  = st.number_input("📊 कंपनी रेट (₹/क्विंटल)", min_value=0.0, step=1.0)
                with r2c4: owner_rate = st.number_input("🚛 गाड़ी वाला रेट (₹/क्विंटल)", min_value=0.0, step=1.0)
                ...
                submitted = st.form_submit_button("➡️ सेव करने के लिए आगे बढ़ें", use_container_width=True, type="primary")
                ...
                if submitted:
                    if not truck_no or not to_loc:
                        ...
                        st.session_state.temp_data = {
                            ...
                            "truck_no": truck_no, "to_loc": to_loc, "gr_no": gr_no, "owner_rate": owner_rate,
        ...
        if st.session_state.show_confirm:
            ...
            st.markdown(f"""
                <div class='confirm-box'>
                    ...
                    <small>
                        📍 {d['from_loc']} → {d['to_loc']} &nbsp;|&nbsp; ⚖️ {d['weight']} क्विंटल &nbsp;|&nbsp;
                        💵 कंपनी भाड़ा: ₹{d['comp_freight']:,} &nbsp;|&nbsp; 🚛 गाड़ी भाड़ा: ₹{d['owner_freight']:,}
                    </small>
                </div>
            ...
            if cb1.button("👍 हाँ, सेव करें", type="primary"):
                if st.session_state.bk_saving_lock: st.toast("⏳ प्रोसेस हो रहा है...")
                else:
                    st.session_state.bk_saving_lock = True
                    with st.spinner("⏳ सुपरफ़ास्ट डेटा सेव हो रहा है..."):
                        ...
                        # Dictionary for Supabase
                        data_dict = {
                            "date_val": str(d['b_date']), "from_loc": str(d['from_loc']), "company": str(d['company']),
                            "owner_rate": float(d['owner_rate']), "comp_rate": float(d['comp_rate']), "weight": float(d['weight']),
                            "truck_no": str(d['truck_no']), "to_loc": str(d['to_loc']), "gr_no": str(d['gr_no']) if d['gr_no'] else "N/A",
                            "uni_amt": int(d['universal_amt']), "comments": str(d['comments']), "comp_freight": int(d['comp_freight']),
                            "owner_freight": int(d['owner_freight']), "final_uni_amt": final_uni_amt, "trip_id": trip_id,
                            "ish_amt": int(d['ishtyaque_amt']), "gr_link": None
                        }
                        
                        if save_booking_to_db(data_dict):
                            ...
                            st.cache_data.clear()
                            st.success(f"✅ गाड़ी {d['truck_no']} की बुकिंग 1 सेकंड में सेव हो गई!")
                            time.sleep(1.5)
    ...
    with tab2:
        ...
        if df_trips.empty:
            st.info("कोई पुरानी बुकिंग नहीं मिली। (यहाँ पुरानी शीट का डेटा आना बाकी है)")
        else:
            ...
            selected_label = st.selectbox("✏️ एडिट करने के लिए गाड़ी चुनें:", ["चुनें..."] + labels)
            st.markdown("<hr style='margin:0.5em 0;border-color:#e2e8f0'>", unsafe_allow_html=True)
            ...
            if selected_label != "चुनें...":
                ...
                with col_edit:
                    ...
                    with st.form("edit_booking_form"):
                        ...
                        def s_str(val): return str(val) if pd.notna(val) and str(val).lower() != "nan" else ""
                        ...
                        if st.form_submit_button("💾 अपडेट करें", use_container_width=True, type="primary"):
                            with st.spinner("अपडेट हो रहा है..."):
                                ...
                                updated_dict = {
                                    "date_val": str(e_date), "from_loc": str(e_from), "company": str(e_company),
                                    "owner_rate": e_owner_rate, "comp_rate": e_comp_rate, "weight": e_weight,
                                    "truck_no": str(e_truck), "to_loc": str(e_to), "gr_no": final_gr,
                                    "uni_amt": e_uni_amt, "comments": str(e_comments),
                                    "comp_freight": e_comp_freight, "owner_freight": e_owner_freight,
                                    "final_uni_amt": e_final_uni, "ish_amt": e_ish_amt
                                }
                                
                                if update_booking_in_db(selected_trip_id, updated_dict):
                ...
                with col_gr:
                    ...
                    if (len(row_data) > 16 and pd.notna(row_data.iloc[16]) and "http" in str(row_data.iloc[16])):
                        ...
                        st.markdown("<div style='background:#d1e7dd;border:1px solid #0f5132;border-radius:8px;padding:6px 12px;font-size:0.82rem;color:#0f5132;font-weight:bold;margin-bottom:8px;'>✅ GR सुरक्षित है</div>", unsafe_allow_html=True)
                        st.link_button("📥 GR कॉपी देखें", existing_gr_url, type="secondary", use_container_width=True)
                    else:
                        st.markdown("<div style='background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:6px 12px;font-size:0.82rem;color:#856404;font-weight:bold;margin-bottom:8px;'>⚠️ GR अभी तक अपलोड नहीं</div>", unsafe_allow_html=True)

                    gr_files = st.file_uploader("GR फोटो (A4 में सेव होगी)", type=["pdf", "jpg", "jpeg", "png"], accept_multiple_files=True, key=f"gr_up_{selected_trip_id}", label_visibility="collapsed")
                    if gr_files: st.caption(f"📎 {len(gr_files)} फ़ाइल चुनी गई")
    ...
    # ══════════════════════════════════════
    with tab3:
        st.markdown("### 📑 Excel से बल्क बुकिंग अपलोड (रॉकेट स्पीड)")

        template_cols = ["Date (YYYY-MM-DD)", "From", "Company", "Owner Rate", "Company Rate", "Weight", "Truck No", "To", "GR No", "Universal Amt", "Comments", "Ishtyaque Profit"]
        output = io.BytesIO()
        ...
        with col_dl:
            st.download_button("⬇️ Excel Template डाउनलोड करें", data=output.getvalue(), file_name="Khan_Transport_Bulk_Format.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)

        st.divider()
        uploaded_excel = st.file_uploader("📥 भरी हुई Excel/CSV अपलोड करें", type=["xlsx", "xls", "csv"])
        ...
        if uploaded_excel is not None:
            try:
                ...
                if st.button("🚀 सभी गाड़ियाँ सेव करें", type="primary", use_container_width=True):
                    success_count, error_count = 0, 0
                    progress_bar = st.progress(0, text="सेव हो रही हैं...")

                    with st.spinner(f"⏳ {len(df_upload)} गाड़ियाँ सेव हो रही हैं... (Supabase Fast Engine)"):
                        total = len(df_upload)
                        ...
                        for index, row in df_upload.iterrows():
                            try:
                                ...
                                data_dict = {
                                    "date_val": date_str, "from_loc": from_loc, "company": company,
                                    "owner_rate": owner_rate, "comp_rate": comp_rate, "weight": weight,
                                    "truck_no": truck_no, "to_loc": to_loc, "gr_no": gr_no if gr_no else "N/A",
                                    "uni_amt": int(uni_amt), "comments": comments, "comp_freight": comp_freight,
                                    "owner_freight": owner_freight, "final_uni_amt": final_uni_amt,
                                    "trip_id": trip_id, "ish_amt": int(ish_amt), "gr_link": None
                                }
                    ...
                    st.cache_data.clear()
                    if success_count > 0: st.success(f"🎊 {success_count} गाड़ियाँ रॉकेट स्पीड से सेव हो गईं!")
                    if error_count > 0: st.warning(f"⚠️ {error_count} rows skip हुईं।")
