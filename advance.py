    ...
    with tab1:
        if not st.session_state.show_confirm:
            with st.form(key=f"booking_form_{c}"):
                # 🟢 TAB 1 को भी 3-कॉलम में कर दिया गया है (बिल्कुल Tab 2 की तरह)
                ...
                with r2c1: to_loc = st.text_input("कहाँ तक")
                with r2c2: company = st.selectbox("कंपनी", ["Universal Industries", "Other"])
                with r2c3: weight = st.number_input("माल का वज़न", min_value=0, step=1)

                r3c1, r3c2, r3c3 = st.columns(3)
                with r3c1: comp_rate = st.number_input("कंपनी रेट", min_value=0, step=1)
                with r3c2: owner_rate = st.number_input("गाड़ी वाला रेट", min_value=0, step=1)
                with r3c3: universal_amt = st.number_input("Universal (₹)", min_value=0, value=1000, step=10)

                r4c1, r4c2, r4c3 = st.columns(3)
                with r4c1: ishtyaque_amt = st.number_input("Ishtyaque (₹)", min_value=0, value=0, step=100)
                with r4c2: gr_no = st.text_input("GR Number (Optional)")
                with r4c3: comments = st.text_input("टिप्पणी")
                ...
                mc2.metric("एडवांस (90%)", f"f₹{advance_approx:,}")
