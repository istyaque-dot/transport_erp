...
def show_receivable_page():
    ...
    if df.empty:
        st.info("कोई बुकिंग नहीं मिली। पहले गाड़ी लोड करें।"); return
    ...
    if selected == "चुनें...":
        st.info("👆 ऊपर से गाड़ी चुनें।"); return
    ...
    col_m2.metric("📉 TDS (1%)", f"- ₹{tds_amount:,}")
    col_m3.metric("📥 अब तक आया", f"₹{already_received:,}")
    col_m4.metric("⏳ बाकी बैलेंस", f"₹{max(0, pending_balance):,}")

    if pending_balance <= 0:
        st.markdown("<div class='status-clear'>✅ हिसाब पूरा हो चुका है।</div>", unsafe_allow_html=True); return
