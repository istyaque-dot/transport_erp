...
def show_reports_page():
    ...
    # 🟢 नया: "डॉक्यूमेंट प्रिंट" नाम का 5वाँ टैब जोड़ दिया गया है
...
----------------------------"""
    ...
    # --- TAB 5: 🟢 NAYA TAB (GR & POD PRINT) ---
    with tab5:
        st.markdown("### 🖨️ डॉक्यूमेंट प्रिंट (GR और POD कॉपी)")
        st.write("GR नंबर सर्च करें और गाड़ी के कागज़ात देखें या प्रिंट निकालें।")
        
        all_bk = get_sheet_data_for_reports("Bookings")
        if len(all_bk) > 1:
            data_bk = all_bk[1:][::-1]
            
            # सिर्फ वो गाड़ियां जिनमें GR नंबर डला हुआ है
            gr_options = []
            for r in data_bk:
                gr = str(r[8]).strip()
                if gr and gr != "N/A" and gr.lower() != "nan":
                    gr_options.append(f"GR: {gr} | 🚛 {r[6]} | 📅 {r[0]} | ID: {r[14]}")
            
            if not gr_options:
                st.info("कोई GR नंबर सिस्टम में नहीं मिला।")
            else:
                selected_gr = st.selectbox("🔍 GR नंबर सर्च करें:", ["चुनें..."] + gr_options)
                
                if selected_gr != "चुनें...":
                    sel_id = selected_gr.split("ID: ")[1].strip()
                    trip_row = [r for r in data_bk if r[14] == sel_id][0]
                    ...
                    with col1:
                        ...
                        if gr_link:
                            st.success("✅ GR की फोटो उपलब्ध है")
                            st.link_button("🖨️ GR कॉपी देखें / प्रिंट करें", gr_link, type="primary")
                        else:
                            st.warning("⚠️ इस गाड़ी की GR कॉपी अभी अपलोड नहीं की गई है।")
                            
                    # POD (रिसीविंग) सेक्शन
                    with col2:
                        st.subheader("🏁 POD (रिसीविंग) कॉपी")
                        if pod_link:
                            st.success("✅ POD की फोटो उपलब्ध है")
                            st.link_button("🖨️ POD कॉपी देखें / प्रिंट करें", pod_link, type="primary")
                        else:
                            st.warning("⚠️ इस गाड़ी की POD अभी अपलोड नहीं की गई है।")
